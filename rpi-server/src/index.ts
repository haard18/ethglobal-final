...
// Initialize market data service connection
console.log('🔄 Initializing market data connection...');
import('./services/marketDataService.js').then(({ marketDataService }) => {
    marketDataService.checkHealth().then((isHealthy) => {
        if (isHealthy) {
            console.log('✅ Market data service connected successfully');
        } else {
            console.warn('⚠️  Market data service not accessible - some features may be limited');
        }
    });
});
...
app.get("/market/overview", async (req: Request, res: Response) => {
    try {
...
app.get("/market/token/:address/price", async (req: Request, res: Response) => {
    try {
        const { address } = req.params;
        ...
        const { marketDataService } = await import('./services/marketDataService.js');
        const priceData = await marketDataService.getTokenPrice(address);
...
app.get("/market/token/:address/stats", async (req: Request, res: Response) => {
    try {
        const { address } = req.params;
        ...
        const { marketDataService } = await import('./services/marketDataService.js');
        const stats = await marketDataService.getTokenStats(address);
        
        if (!stats) {
            return res.status(404).json({
                success: false,
                error: "Token stats not available"
            });
        }
...
// Recent market activity endpoint
app.get("/market/activity/recent", async (req: Request, res: Response) => {
    try {
        const limit = parseInt(req.query.limit as string) || 10;
        const eventType = req.query.type as string;
        
        const { marketDataService } = await import('./services/marketDataService.js');
        const activity = await marketDataService.getRecentEvents(Math.min(limit, 50), eventType);
...
app.get("/market/top/:type", async (req: Request, res: Response) => {
    try {
        ...
        let tokens;
        if (type === 'gainers') {
            tokens = await marketDataService.getTopGainers(limit);
        } else if (type === 'losers') {
            tokens = await marketDataService.getTopLosers(limit);
        } else {
            return res.status(400).json({
                success: false,
        ...
        console.error(`Error getting top ${req.params.type}:`, error);
        res.status(500).json({
            success: false,
            error: `Failed to get top ${req.params.type}`,
            message: error instanceof Error ? error.message : "Unknown error"
        });
    }
});

// User trading activity endpoint
app.get("/market/user/:address/activity", async (req: Request, res: Response) => {
    try {
        const { address } = req.params;
        ...
        const { marketDataService } = await import('./services/marketDataService.js');
        const userActivity = await marketDataService.getUserEvents(address, limit);
        
        if (!userActivity) {
            return res.status(404).json({
                success: false,
                error: "No trading activity found for this user"
            });
        }
...
app.get("/market/health", async (req: Request, res: Response) => {
    try {
        const { marketDataService } = await import('./services/marketDataService.js');
